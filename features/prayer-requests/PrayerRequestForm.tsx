  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-gold/20 bg-gold/5 px-6 py-12 text-center">
        ...
        <div>
          <p className="font-semibold text-gray-900">{t.prayerReceivedTitle}</p>
          <p className="mt-1 text-sm text-gray-500">{t.prayerReceivedDesc}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label className={labelClass}>{t.emailFieldLabel}</label>
          <input
            {...register("email")}
            type="email"
      ...
      <div>
        <label className={labelClass}>
          {t.prayerNeedLabel} <span className="text-red-500">*</span>
        </label>
        <textarea
          {...register("message")}
          rows={5}
          placeholder={t.prayerNeedPlaceholder}
      ...
      <div className="flex items-start gap-3 rounded-lg bg-gray-50 p-4">
        ...
        <label htmlFor="isPrivate" className="text-sm text-gray-600 leading-relaxed">
          <span className="font-medium text-gray-700">{t.privateLabel}</span> — {t.privateDesc}
        </label>
      </div>
      ...
      >
        {isPending ? t.sendingPrayer : t.submitPrayer}
      </button>
    </form>
  );
}
