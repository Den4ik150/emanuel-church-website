...
export default async function AdminSchedulePage() {
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <div className="rounded-xl border border-gray-200 bg-white p-12 text-center text-sm text-gray-500">Расписания пока нет</div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600">День</th>
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <LockedEditLink href={`/admin/schedule/${item.id}/edit`} itemStream={item.stream} adminStream={adminStream} />
                      <LockedDeleteButton itemStream={item.stream} adminStream={adminStream} deleteAction={deleteScheduleItem.bind(null, item.id)} confirmText="Удалить эту запись расписания?" />
                    </div>
                  </td>
                </tr>
              ))}
