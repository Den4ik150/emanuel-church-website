...
export default async function AdminEventsPage() {
  ...
  return (
    <div>
      ...
      {events.length === 0 ? (
        <div className="rounded-xl border border-gray-200 bg-white p-12 text-center text-sm text-gray-500">Мероприятий пока нет</div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600">Место</th>
              ...
              {events.map((event) => (
                <tr key={event.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 font-medium text-gray-900">
                    {event.title}
                    {event.isFeatured && <span className="ml-2 inline-flex rounded-full bg-gold/10 px-2 py-0.5 text-xs font-medium text-gold">Главная</span>}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <LockedEditLink href={`/admin/events/${event.id}/edit`} itemStream={event.stream} adminStream={adminStream} />
                      <LockedDeleteButton itemStream={event.stream} adminStream={adminStream} deleteAction={deleteEvent.bind(null, event.id)} confirmText="Удалить это мероприятие?" />
                    </div>
                  </td>
                </tr>
              ))}
