...
export default async function AdminPagesPage() {
  ...
  return (
    <div>
      ...
      {pages.length === 0 ? (
        <div className="rounded-xl border border-gray-200 bg-white p-12 text-center text-sm text-gray-500">Страниц пока нет</div>
      ) : (
                ...
                <tr key={page.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <LockedEditLink href={`/admin/pages/${page.id}/edit`} itemStream={page.stream} adminStream={adminStream} />
                      <LockedDeleteButton itemStream={page.stream} adminStream={adminStream} deleteAction={deletePage.bind(null, page.id)} confirmText="Удалить эту страницу?" />
                    </div>
                  </td>
                </tr>
              ))}
